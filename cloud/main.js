...
Parse.Cloud.define("cloudservertest", function(request, response){
  ...
  Parse.Promise.as().then(function() {
   ...
   return itemQuery.first().then(null, function(error) {
    ...
    // Create new order
    return general.save().then(null, function(error) {
      console.log('Incrementing order count failed. Error: ' + error);
      return Parse.Promise.error('Error - Imcrement not registered, Please contact Us');
    });
  }).then(function(result) {
   // Proceed to create the order
    order = new Parse.Object('Orders');
...
Parse.Cloud.define("onlyorder", function(request, response){
  ...
  Parse.Promise.as().then(function() {
   // Proceed to create the order
    ...
    order.set('amount', request.params.amount);
    order.set('Completed', false);  
    function request.params.voucher(bool){
